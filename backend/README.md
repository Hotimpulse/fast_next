...
- В API список `/employees` показывает текущие неудаленные назначения, и экспортная секция `employees` использует эти данные.
...
```text
GET    /employees?search=&department_id=&division_id=&status=&cutoff=&limit=&offset=
...
Очищает прикладные таблицы. Используется кнопкой на главной странице frontend части приложения.
