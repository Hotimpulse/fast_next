...
from models.employee import AssignmentVersion, EmployeeAssignment, OrganizationUnit, Person
...
def _base_employee_query(db: Session, cutoff: date):
    return (
        ...
        .filter(AssignmentVersion.effective_from <= cutoff)
...
def list_employees(
    ...
    *,
    ...
    target_date = cutoff or _today()
    query = _base_employee_query(db, target_date)
...
def get_employee(db: Session, assignment_id: str, *, cutoff: date | None = None) -> EmployeeRead:
    target_date = cutoff or _today()
    row = (
        _base_employee_query(db, target_date)
...
def create_employee(db: Session, payload: EmployeeCreate) -> EmployeeRead:
    ...
    person = _get_or_create_person(db, payload.full_name)
    ...
    version = AssignmentVersion(
        ...
        salary=Decimal(str(payload.salary)) if payload.salary is not None else None,
...
def patch_employee(db: Session, assignment_id: str, payload: EmployeePatch) -> EmployeeRead:
    ...
    if not current:
        raise HTTPException(status_code=404, detail="Current employee version not found.")
    ...
    if termination_date and termination_date < hire_date:
        raise HTTPException(status_code=422, detail="termination_date cannot be earlier than hire_date.")
    ...
    version = AssignmentVersion(
        ...
        salary=(
            ...
            if "salary" in fields and payload.salary is not None
            else None
            if "salary" in fields
            else current.salary
        ),
